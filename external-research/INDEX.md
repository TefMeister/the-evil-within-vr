...
**Last `/gr` pass: 2026-09-02 (scoped run, second pass same day) — FULL** (dossier §7/§8/§12 read in full for research targets; today's earlier CHECK-IN already covered the three OPEN rows correctly as non-research items)**.** Inbox empty. One new topic: domain-shader-stage per-eye stereo transforms are documented, patented prior art (NVIDIA US10068366B2) — a real template for the §8/§12 tessellation coverage gap, not a cold-start problem. The other research target (whether id Tech 5's job system structurally guarantees non-concurrent cross-thread buffer writes) came back with nothing public — the SIGGRAPH deck is image-only and every secondary source is too shallow; stays exactly where the dossier already had it.
_Previous: Last `/gr` pass: 2026-09-02 (estate sweep) — CHECK-IN (board OPEN block + INDEX; no dossier or topic re-read). Inbox empty. Nothing new — the open rows are a branch-merge decision, a witnessed runtime proof and a bytecode-saving launch, none of them a public-research question._
